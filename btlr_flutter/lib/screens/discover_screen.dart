...
class _DiscoverScreenState extends ConsumerState<DiscoverScreen> with SingleTickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      body: TabBarView(
        ...
        children: [
          ...
          _buildProgressTab(activityDashboardAsync),
        ],
      ),
  ...
  Widget _buildProgressTab(AsyncValue<Map<String, dynamic>> dashboardAsync) {
    return dashboardAsync.when(
      data: (dashboard) {
        final leetcode = dashboard['leetcode'] as Map<String, dynamic>?;
        final github = dashboard['github'] as Map<String, dynamic>?;
        final summary = dashboard['summary'] as Map<String, dynamic>;

        return RefreshIndicator(
          color: kPrimaryBlue,
          onRefresh: () => ref.read(activityDashboardProvider.notifier).loadDashboard(),
          child: ListView(
            padding: const EdgeInsets.all(16),
            children: [
              // Summary Card
              ...
              if (leetcode != null) ...[
                _ActivityCard(
                  ...
                  color: Colors.orange,
                  data: leetcode,
                  onSync: () => ref.read(activityDashboardProvider.notifier).syncPlatform('leetcode'),
                ),
                const SizedBox(height: 16),
              ],
              ...
              if (github != null) ...[
                _ActivityCard(
                  ...
                  data: github,
                  onSync: () => ref.read(activityDashboardProvider.notifier).syncPlatform('github'),
                ),
                const SizedBox(height: 16),
              ],

              // Setup prompt if no trackers
              if (leetcode == null && github == null)
                Center(
                  child: Column(
                    children: [
                      Icon(
                      ...
                      TextButton(
                        onPressed: () => _showSetupDialog(context),
                        child: const Text('TAP TO SETUP NOW'),
                      ),
                    ],
                  ),
                ),
