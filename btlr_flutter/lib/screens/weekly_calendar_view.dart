...
//     final weeklyBlocksAsync = ref.watch(calendarBlocksProvider(7));
...
class _WeeklyCalendarViewState extends ConsumerState<WeeklyCalendarView> {
  ...
  DateTime _weekStart = _normalizeDate(DateTime.now().subtract(Duration(days: DateTime.now().weekday - 1)));
  ...
  void _goToToday() {
    setState(() {
      _weekStart = _normalizeDate(DateTime.now().subtract(Duration(days: DateTime.now().weekday - 1)));
    });
    // ✅ Force provider refresh
    ref.invalidate(weeklyBlocksProvider);
  }
...
Widget build(BuildContext context) {
    ...
    body: Column(
      children: [
        ...
        Expanded(
          child: weeklyBlocksAsync.when(
            data: (blocks) {
              print('📅 Loaded ${blocks.length} blocks for ${startDate.toIso8601String()} to ${endDate.toIso8601String()}');
              return _buildCalendarGrid(weekDays, blocks);
            },
            loading: () => const Center(child: CircularProgressIndicator()),
            error: (error, stack) {
  ...
  // (Copy all the _buildWeekNavigation, _buildCalendarGrid, etc. methods exactly as they are)
