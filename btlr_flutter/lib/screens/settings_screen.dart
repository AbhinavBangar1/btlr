                  ...
                  child: _SettingsSection(
                    ...
                    children: [
                      _SettingsTile(
                        icon: Icons.code,
                        title: 'GitHub',
                        subtitle: student.githubUsername ?? 'Not set',
                        onTap: () => _showTextEditDialog(
                          context, 
                          ref, 
                          'GitHub Username', 
                          student.githubUsername ?? '',
                          (username) async => await ref.read(studentProfileProvider.notifier).updateProfile(githubUsername: username),
                        ),
                      ),
                      _SettingsTile(
                        icon: Icons.terminal,
                        title: 'LeetCode',
                        subtitle: student.leetcodeUsername ?? 'Not set',
                        onTap: () => _showTextEditDialog(
                          context, 
                          ref, 
                          'LeetCode Username', 
                          student.leetcodeUsername ?? '',
                          (username) async => await ref.read(studentProfileProvider.notifier).updateProfile(leetcodeUsername: username),
                        ),
                      ),
                      _SettingsTile(
                        icon: Icons.emoji_events,
                        title: 'Codeforces',
                        subtitle: student.codeforcesUsername ?? 'Not set',
                        onTap: () => _showTextEditDialog(
                          context, 
                          ref, 
                          'Codeforces Username', 
                          student.codeforcesUsername ?? '',
                          (username) async => await ref.read(studentProfileProvider.notifier).updateProfile(codeforcesUsername: username),
                        ),
                      ),
                      _SettingsTile(
                        icon: Icons.work,
                        title: 'LinkedIn',
                        subtitle: student.linkedinUrl ?? 'Not set',
                        onTap: () => _showTextEditDialog(
                          context, 
                          ref, 
                          'LinkedIn URL', 
                          student.linkedinUrl ?? '',
                          (url) async => await ref.read(studentProfileProvider.notifier).updateProfile(linkedinUrl: url),
                        ),
                      ),
                      _SettingsTile(
                        icon: Icons.web,
                        title: 'Portfolio',
                        subtitle: student.portfolioUrl ?? 'Not set',
                        onTap: () => _showTextEditDialog(
                          context, 
                          ref, 
                          'Portfolio URL', 
                          student.portfolioUrl ?? '',
                          (url) async => await ref.read(studentProfileProvider.notifier).updateProfile(portfolioUrl: url),
                        ),
                      ),
                    ],
                  ),
